...
const Quest = () => {
	...
	const incrementIndex = async () => {
		if (index < 4) {
			...
			if (
				...
				try {
					const userDocRef = doc(db, 'users-beta', id);
					await setDoc(
						userDocRef,
						{
							email,
					...
					navigate('/thanks-for-reply');
				} catch (error) {
					toast.error('Intente mas tarde por favor.');
					console.error(error);
				}
			}
		}
	...
	return (
		<>
			<div className="bg-primary w-full h-screen flex justify-between p-5 sm:p-14 items-center font-inter">
